...
void DataInterface::Init(ros::NodeHandle* nh_ptr, const std::string& name,
...
void DataInterface::ParameterInit() {
  nh_ptr_->param<std::string>("sensor_frame", ksensor_frame_, "lidar");
  nh_ptr_->param<int32_t>("sensor_lines", ksensor_lines_, 16);
  nh_ptr_->param<int32_t>("sensor_scans", ksensor_scans_, 1800);

  std::vector<double> sensor_position_vec;
  std::vector<double> sensor_orientation_vec;
  nh_ptr_->param<std::vector<double>>("sensor_position", sensor_position_vec,
                                      std::vector<double>({0.0, 0.0, 0.0}));
  nh_ptr_->param<std::vector<double>>(
